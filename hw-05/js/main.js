 ...
 */
const getUsersByFriend = (arr, name) => arr.filter(user => user.friends.some(prop => prop === name)).map(user => user.name);
...
const getAllUniqueSkills = arr => arr.reduce((acc, user) => user.skills ? acc.concat(user.skills) : acc, []).filter((skill, index, arr) => arr.indexOf(skill) === index).sort();
...
const getUserNamesSortedByFriendsCount = arr => arr.sort(sortByFriends).map(user => user.name);

...
function createLi () {
  preview.appendChild(addLi);
}

function createImg (mass) {
  mass.forEach(el => {
    const addLi = document.createElement('li')
    const addImg = document.createElement('img');
    addImg.setAttribute('src', el.preview);
    addImg.setAttribute('data-fullview', el.fullview);
    addImg.setAttribute('alt', el.alt);
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  gallery.addEventListener('click', changeSrc);
  function changeSrc (event) {
    const target = event.target;
    const targetFullview = target.dataset.fullview;
    const changeAtr = function () {
      if (target.parentNode.nodeName === 'BODY' || target.parentNode.nodeName === 'DIV') return;
      fullview.src = targetFullview;
      };
      return  changeAtr() ;
  }
});
